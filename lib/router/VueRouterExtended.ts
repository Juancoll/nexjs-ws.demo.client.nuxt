...
export class VueRouterExtended extends Router {
    ...
    public get isAuthRoute (): boolean {
      ...
      const result =
            this._branches.authenticate.auth.routes
              .map(x => x.path)
              .includes(this.currentRoute.path)
      return result
    }
    // #endregion
    ...
    constructor (props: IVueRouterExtendedProps) {
      ...
      if (process.client) { console.log(location.hash) }

      this._existingPath = this.getHash()

      this.useAuth = !!props.auth
    ...
    public get userValidRoles (): string[] {
      return !this._user || !this._user.roles
        ? []
        : this._user.roles.filter(x => this.roles.includes(x))
    }
    ...
    public updateRole (role?: string): void {
      ...
      const useExistingRoute =
            this._existingPath &&
            this.findRoute(this._existingPath) !=
                undefined
